...
if (!class_exists( 'EventusTeamPicture') && class_exists('aviaShortcodeTemplate')) {
	class EventusTeamPicture extends aviaShortcodeTemplate {
		...
		function shortcode_handler($atts, $content = "", $shortcodename = "", $meta = "") {	
			...
			$this->context['img'] = [
				'src' => (!$myTeam->getImg() ? plugin_dir_url( __FILE__ ).'../../../includes/img/img-default.png' : wp_get_attachment_image_src($myTeam->getImg(), $format)[0]),
				'attachment' => $myTeam->getImg(),
				'format' => $format
			];

			return \Timber\Timber::fetch("teamPicture.twig", $this->context);
	    }
	}
}
